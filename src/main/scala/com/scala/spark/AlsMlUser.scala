...
class AlsMlUser() extends Serializable{
  ...
  def alsModelingPackage(): Unit = {
    ...
    val df = sparkSession.read.option("header","true").csv("hdfs://localhost:8020/Recomender/data2.csv")
    df.show()
    var base_df = df
   ...
   // var dataSource = "hdfs://localhost:8020/Recomender/data.csv"
    ...
    var dataHeaderPresent = true
     ...
     // var sc = SparkSession.builder.master("local").appName("Spark SQL basic example").getOrCreate()
      ...
      var userRecommended = model.recommendProductsForUsers(noOfRecommendations)
     ...
     /* outputpathalsMetric.put(path2, alsMetric)
      ...
      ALSAttributes().setOutputpathModelLinkedHashMap(outputpathModelLinkedHashMap)*/
        ...
        var path="hdfs://localhost:8020/AlsOutput/UserResult"
 
      writeToHDFS(sparkSession,userRecommendedDf,"true",path)
    
  }
   def writeToHDFS(sparkSession: SparkSession, df_encoded: DataFrame, isHeaderAvailable: String, outputStorageDir: String): Unit = {
    println("inside hdfs")
     df_encoded.show()
     println( "   "  )
    ...
    // rdddf.collect().foreach(println)
     val wordcount = rdddf.map(line => (line.split("[")))
     println("wordcount")
